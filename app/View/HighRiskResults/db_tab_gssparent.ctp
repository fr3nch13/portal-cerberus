...
foreach($fismaSystems as $fismaSystem)
{
	...
	$stats['FismaSystem.'.$id] = array(
		...
		'value' => count($fismaSystem['HighRiskResults']),
		'color' => substr(md5($fismaSystem['FismaSystem']['name']), 0, 6),
	);
	
	$stats['total']['value'] = ($stats['total']['value'] + count($fismaSystem['HighRiskResults']));
}

$stats = Hash::sort($stats, '{s}.value', 'desc');
$pie_data = array(array(__('GSS Parent'), __('num %s', __('High Risk Results')) ));
...
echo $this->element('Utilities.object_dashboard_block', array(
	'title' => __('%s by %s', __('High Risk Results'), __('GSS Parent')),
