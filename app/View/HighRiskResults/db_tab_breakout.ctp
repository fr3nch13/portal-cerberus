...
// tab-hijack
$page_options = array();
$page_options['org'] = $this->Html->link(__('By %s', __('ORG/IC')), array('action' => 'db_tab_breakout', 'org', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));
$page_options['division'] = $this->Html->link(__('By %s', __('Division')), array('action' => 'db_tab_breakout', 'division', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));
$page_options['branch'] = $this->Html->link(__('By %s', __('Branch')), array('action' => 'db_tab_breakout', 'branch', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));
$page_options['sac'] = $this->Html->link(__('By %s', __('SAC')), array('action' => 'db_tab_breakout', 'sac', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));
$page_options['owner'] = $this->Html->link(__('By %s', __('System Owner')), array('action' => 'db_tab_breakout', 'owner', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));
$page_options['fisma_system'] = $this->Html->link(__('By %s', __('FISMA System')), array('action' => 'db_tab_breakout', 'fisma_system', $as_block, 'reports_status_id' => $reportsStatusId), array('class' => 'tab-hijack block-hijack'));

$allUrl = array('action' => $this->action, 'reports_status_id' => 0);
...
foreach($results as $resultId => $result)
{
	...
	$td[$resultId]['name'] = $this->Html->link($result['name'], $result['url'], array('class' => 'embiggen'));
	$path = false;
...
if($as_block)
{
	...
	echo $this->element('Utilities.object_dashboard_block', array(
		'title' => __('%s - %s grouped by %s', $reportsStatusName, __('Results'), $scopeName),
		'subtitle' => $subtitle,
