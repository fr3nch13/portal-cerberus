...
$page_options2 = [];
$page_options2['ReportsStatus.0'] = $this->Html->link($subtitle, $this->Html->urlModify(['reports_status_id' => 0]), ['class' => 'tab-hijack block-hijack']);
foreach($reportsStatuses as $reportsStatus)
{
	$reportsStatus_id = $reportsStatus['ReportsStatus']['id'];
	$page_options2['ReportsStatus.'.$reportsStatus_id] = $this->Html->link(__('By Status: %s', $reportsStatus['ReportsStatus']['name']), $this->Html->urlModify(['reports_status_id' => $reportsStatus_id]), ['class' => 'tab-hijack block-hijack']);
...
if($as_block)
{
	...
	echo $this->element('Utilities.object_dashboard_block', [
		'title' => __('%s - %s grouped by %s', __('FOV'), __('Totals'), $scopeName),
		'subtitle' => $subtitle,
