...
$page_options = array();
if($this->Common->roleCheck(array('admin', 'saa')) and isset($pocs) and !$isSubscription)
{
	foreach($pocs as $poc_id => $poc_name)
	{
		$page_options[$poc_id] = $this->Html->link($poc_name, array('action' => $this->request->action, $poc_id));
	}
}
...
foreach($fismaSystemsAll as $fismaSystem)
{
	// track the fisma systems
	if(isset($fismaSystem['FismaSystem']['id']) and !isset($fismaSystemsIndex[$fismaSystem['FismaSystem']['id']]))
		$fismaSystemsIndex[$fismaSystem['FismaSystem']['id']] = $fismaSystem['FismaSystem'];
...
//		$ralis[$y] = $this->Html->tag('h4', $this->Html->link($fismaSystemIndex['name'], '#fisma-system-'. $fismaSystem['FismaSystem']['id'])).$this->Html->tag('p', $fismaSystemIndex['fullname']);
	...
	$content[] = $this->Html->tag('p', __('The list of all %s that have you listed as one of the points of contact.', __('FISMA Systems')), array('class' => 'page-description'));
...
echo $this->element('Utilities.page_generic', array(
	'page_title' => __('Unresolved Results for: %s', $adAccount['AdAccount']['name']),
