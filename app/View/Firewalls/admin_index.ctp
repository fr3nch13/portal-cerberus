...
// content
$th = array(
	'Firewall.name' => array('content' => __('Name'), 'options' => array('sort' => 'Firewall.name')),
	...
	'Firewall.slug' => array('content' => __('Slug'), 'options' => array('sort' => 'Firewall.slug')),
...
foreach ($firewalls as $i => $firewall)
{
	$td[$i] = array(
		...
		array(
			...
			$this->Html->link(__('Delete'), array('action' => 'delete', $firewall['Firewall']['id']),array('confirm' => 'Are you sure?')), 
			array('class' => 'actions'),
		),
	);
}
