<?php 
// File: app/View/Divisions/byorg.ctp

$page_title = __('Orginizations');
if($contactType)
{
	$page_title = __('%s by %s', __('Orginizations'), $contactType);
}
$page_options = array();
...
if($contactType)
{
	$th['count_divisions'] = array('content' => __('# %s', __('Divisions')));
...
foreach ($orgs as $i => $org)
{
	...
	if($contactType)
	{
		$td[$i]['count_divisions'] = $org['counts']['divisions'];
		if(!isset($totals['count_divisions'])) $totals['count_divisions'] = 0;
		$totals['count_divisions'] = ($totals['count_divisions'] + $td[$i]['count_divisions']);
		if($td[$i]['count_divisions'])
			$td[$i]['count_divisions'] = array($td[$i]['count_divisions'], array('class' => 'highlight bold'));
		
		$td[$i]['count_ad_accounts'] = $org['counts']['ad_accounts'];
		...
		$td[$i]['fisma_systems'] = $org['counts']['fisma_systems'];
		...
		$td[$i]['parents'] = $org['counts']['parents'];
		...
		$td[$i]['children'] = $org['counts']['children'];
		...
		$td[$i]['pii_count'] = $org['counts']['pii_count'];
		...
		$td[$i]['ongoing_auth_yes'] = $org['counts']['ongoing_auth_yes'];
		...
		$td[$i]['ongoing_auth_no'] = $org['counts']['ongoing_auth_no'];
		...
		/*
		$td[$i]['ongoing_auth_na'] = $org['counts']['ongoing_auth_na'];
		...
		*/
		$td[$i]['fisma_reportable_yes'] = $org['counts']['fisma_reportable_yes'];
		...
		$td[$i]['fisma_reportable_no'] = $org['counts']['fisma_reportable_no'];
		...
		/*
		$td[$i]['fisma_reportable_na'] = $org['counts']['fisma_reportable_na'];
		...
		*/
		foreach($fismaSystemGssStatuses as $id => $name)
		{
			$td[$i]['fismaSystemGssStatus_'. $id] = $org['counts']['fismaSystemGssStatus_'. $id];
		...
		foreach($fismaSystemFipsRatings as $id => $name)
		{
			$td[$i]['fismaSystemFipsRating_'. $id] = $org['counts']['fismaSystemFipsRating_'. $id];
		...
		foreach($fismaSystemRiskAssessments as $id => $name)
		{
			$td[$i]['fismaSystemRiskAssessment_'. $id] = $org['counts']['fismaSystemRiskAssessment_'. $id];
		...
		foreach($fismaSystemThreatAssessments as $id => $name)
		{
			$td[$i]['fismaSystemThreatAssessment_'. $id] = $org['counts']['fismaSystemThreatAssessment_'. $id];
...
{
	$line_count = 0;
	$totals_row = array('name' => __('Totals:'));
	foreach($td[$i] as $k => $v)
	{
		$totals_row[$k] = false;
		if(isset($totals[$k]))
...
$this->set(compact('page_title', 'page_options', 'th', 'td', 'totals_row'));

if(isset($db_block) and $db_block)
{
	$this->extend('db_block_overview');
}
else
{
	$this->extend('Contacts.ContactsDivisions/byorg');
}