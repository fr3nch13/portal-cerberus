<?php 
// File: app/View/Divisions/byshortname.ctp

$page_title = __('%s by %s', __('Divisions'), __('Short Names'));
if($contactType)
{
	$page_title = __('%s by %s by %s', __('Divisions'), __('Short Names'), $contactType);
}
$page_options = array();
...
foreach ($shortnames as $i => $shortname)
{
	...
	if($contactType)
	{
		$td[$i]['count_divisions'] = $shortname['counts']['divisions'];
...
$this->extend('Contacts.ContactsDivisions/byshortname');
