...
foreach ($divisions as $i => $division)
{
	...
	$td[$i]['FismaSystem.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'fisma_systems', 'action' => 'division', $division['Division']['id']), 
		'url' => array('action' => 'view', $division['Division']['id'], 'tab' => 'fisma_systems'),
	));
	$td[$i]['UsResult.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'us_results', 'action' => 'division', $division['Division']['id']), 
		'url' => array('action' => 'view', $division['Division']['id'], 'tab' => 'us_results'),
	));
	$td[$i]['EolResult.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'eol_results', 'action' => 'division', $division['Division']['id']), 
		'url' => array('action' => 'view', $division['Division']['id'], 'tab' => 'eol_results'),
	));
	$td[$i]['PenTestResult.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'pen_test_results', 'action' => 'division', $division['Division']['id']), 
		'url' => array('action' => 'view', $division['Division']['id'], 'tab' => 'pen_test_results'),
	));
	$td[$i]['HighRiskResult.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'high_risk_results', 'action' => 'division', $division['Division']['id']), 
		'url' => array('action' => 'view', $division['Division']['id'], 'tab' => 'high_risk_results'),
	));
	...
	if($contactType)
	{
		...
		$td[$i]['parents'] = $division['counts']['parents'];
		...
		$td[$i]['children'] = $division['counts']['children'];
		...
		$td[$i]['pii_count'] = $division['counts']['pii_count'];
		...
		$td[$i]['ongoing_auth_yes'] = $division['counts']['ongoing_auth_yes'];
		...
		$td[$i]['ongoing_auth_no'] = $division['counts']['ongoing_auth_no'];
		...
		/*
		$td[$i]['ongoing_auth_na'] = $division['counts']['ongoing_auth_na'];
		...
		*/
		$td[$i]['fisma_reportable_yes'] = $division['counts']['fisma_reportable_yes'];
		...
		$td[$i]['fisma_reportable_no'] = $division['counts']['fisma_reportable_no'];
		...
		/*
		$td[$i]['fisma_reportable_na'] = $division['counts']['fisma_reportable_na'];
		...
		*/
		foreach($fismaSystemGssStatuses as $id => $name)
		{
			$td[$i]['fismaSystemGssStatus_'. $id] = $division['counts']['fismaSystemGssStatus_'. $id];
		...
		foreach($fismaSystemFipsRatings as $id => $name)
		{
			$td[$i]['fismaSystemFipsRating_'. $id] = $division['counts']['fismaSystemFipsRating_'. $id];
		...
		foreach($fismaSystemRiskAssessments as $id => $name)
		{
			$td[$i]['fismaSystemRiskAssessment_'. $id] = $division['counts']['fismaSystemRiskAssessment_'. $id];
		...
		foreach($fismaSystemThreatAssessments as $id => $name)
		{
			$td[$i]['fismaSystemThreatAssessment_'. $id] = $division['counts']['fismaSystemThreatAssessment_'. $id];
...
if(isset($i) and isset($td[$i]) and $contactType)
{
	$line_count = 0;
	$totals_row = array(
		'Division.org_id' => __('Totals:'), 
		'Division.shortname' => false,
		'Division.name' => false,
		'Division.director_id' => false,
	);
