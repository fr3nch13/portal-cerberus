...
// tab-hijack
$page_options = array();
$page_options['org'] = $this->Html->link(__('By %s', __('ORG/IC')), array('action' => 'db_tab_breakout', 'org', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
$page_options['division'] = $this->Html->link(__('By %s', __('Division')), array('action' => 'db_tab_breakout', 'division', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
$page_options['branch'] = $this->Html->link(__('By %s', __('Branch')), array('action' => 'db_tab_breakout', 'branch', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
$page_options['sac'] = $this->Html->link(__('By %s', __('SAC')), array('action' => 'db_tab_breakout', 'sac', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
$page_options['owner'] = $this->Html->link(__('By %s', __('System Owner')), array('action' => 'db_tab_breakout', 'owner', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
$page_options['fisma_system'] = $this->Html->link(__('By %s', __('FISMA System')), array('action' => 'db_tab_breakout', 'fisma_system', $as_block, $us_report_id), array('class' => 'tab-hijack block-hijack'));
...
foreach($results as $resultId => $result)
{
	...
	$td[$resultId]['total'] = count($result['UsResults']);
	...
	if(in_array($scope, array('system', 'fisma_system')))
	{
		$td[$resultIdTable]['total'] = array('.', array(
			'colspan' => 2,
			'ajax_content_url' => array('action' => 'fisma_system', $result['id'], $reportsStatusId, 1, $us_report_id), 
		));
	}
	else
	{
		$td[$resultIdTable]['total'] = array('.', array(
			'colspan' => 2,
			'ajax_content_url' => array('action' => $scope, $result['id'], 1, $us_report_id, 'reports_status_id' => $reportsStatusId),
		));
	}
