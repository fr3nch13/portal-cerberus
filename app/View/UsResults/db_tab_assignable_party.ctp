...
foreach($results as $resultId => $result)
{
	...
	foreach($reportsAssignableParties as $reportsAssignableParty_id => $reportsAssignableParty_name)
	{
		...
		{
			if(isset($usResult['EolSoftware']['reports_assignable_party_id']) and $usResult['EolSoftware']['reports_assignable_party_id'] == $reportsAssignableParty_id)
			{
...
if($as_block)
{
	...
	echo $this->element('Utilities.object_dashboard_block', array(
		'title' => __('%s - %s grouped by %s', __('US'), __('Assignable Parties'), $scopeName),
