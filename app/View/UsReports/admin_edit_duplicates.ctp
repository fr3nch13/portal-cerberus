...
?>
<div class="top">
	<h1><?php echo __('Duplicates found in the %s:  %s', __('US Report'), $us_report['UsReport']['name']); ?></h1>
</div>
<div class="center batchform ">
		...
		<ul>
			...
			<li><?php echo __('If you get to a point where there are no records that need to be updated, clicking on the "%s" button will just take you to the details page.', __('Update %s', __('US Results'))); ?></li>
		</ul>
		</div>
		<div class="form">
...
foreach($duplicates as $i => $duplicate)
{
	...
	$input_id = $this->Form->input($id.'.UsResult.id', array('type' => 'hidden', 'value' => $id));
	$input_new = $this->Form->input($id.'.UsResult.new', array('type' => 'hidden', 'value' => 0, 'class' => 'input_new'));
	
	$ip_address_td = $duplicate['ip_address'];
	if($duplicate['ip_address'] != $existing['ip_address'])
	{
		$input_ip_address_existing = $existing['ip_address'];
		
		$input_ip_address_new = $this->Form->input($id.'.UsResult.ip_address', array(
			'type' => 'text', 
	...
	if($duplicate['mac_address'] != $existing['mac_address'])
	{
		$input_mac_address_existing = $existing['mac_address'];
		
		$input_mac_address_new = $this->Form->input($id.'.UsResult.mac_address', array(
			'type' => 'text', 
	...
	if($duplicate['host_name'] != $existing['host_name'])
	{
		$input_host_name_existing = $existing['host_name'];
		
		$input_host_name_new = $this->Form->input($id.'.UsResult.host_name', array(
			'type' => 'text', 
	...
	if($duplicate['netbios'] != $existing['netbios'])
	{
		$input_netbios_existing = $existing['netbios'];
		
		$input_netbios_new = $this->Form->input($id.'.UsResult.netbios', array(
			'type' => 'text', 
	...
	$us_software_td = $eolSoftwares[$existing['eol_software_id']];
	$us_software_td .=  $this->Form->input($id.'.UsResult.eol_software_id', array('type' => 'hidden', 'value' => $existing['eol_software_id']));
	if($duplicate['eol_software_id'] != $existing['eol_software_id'])
	{
		$input_us_software_existing = $eolSoftwares[$existing['eol_software_id']];
		
		$input_us_software_new = $this->Form->input($id.'.UsResult.eol_software_id', array(
			'type' => 'text', 
		...
		$us_software_td = $input_us_software_existing. $input_us_software_new;
	}
	
	$td[] = array(
		$remove. '<br/>'. $new_button,
		$input_id. $input_new.
		$this->Html->link($id, array('controller' => 'us_results', 'action' => 'view', $id), array('target' => 'us_result_existing', 'class' => 'existing_id')),
		$ip_address_td,
		$mac_address_td,
		$host_name_td,
		$netbios_td,
...
echo $this->element('Utilities.table', array(
	...
	'show_refresh_table' => false,
			...
			<?php echo $this->Form->end(__('Update %s', __('US Results'))); ?>
		</div>
</div>
