<?php 
// File: app/View/Emails/text/fisma_status_emails.ctp

$this->Html->setFull(true);
$this->Html->asText(true);

$page_options = array(
	$this->Html->link(_('View these %s', _('FISMA Inventory Items'), array('controller' => 'fisma_inventories', 'action' => 'status', $fisma_status['FismaStatus']['id'])),
);
...
foreach ($fisma_inventories as $i => $fisma_inventory)
{
	...
	$td[$i] = array(
		...
		$this->Html->link($fisma_inventory['FismaSystem']['owner_name'], 'mailto:'. $fisma_inventory['FismaSystem']['owner_email']),
		$this->Html->link($fisma_inventory['FismaSystem']['tech_name'], 'mailto:'. $fisma_inventory['FismaSystem']['tech_email']),
...
echo $this->element('Utilities.email_text_index', array(
	'instructions' => $fisma_status['FismaStatus']['instructions'],
	'page_title' => __('%s Assigned to %s: %s', __('Inventory'), __('FISMA Status'), $fisma_status['FismaStatus']['name']),
