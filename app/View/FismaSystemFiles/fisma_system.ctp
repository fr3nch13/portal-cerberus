...
{
	if($raf)
		$page_options[] = $this->Html->link(__('Add %s ', __('FISMA System %s', $file_type)), array('action' => 'add', $fisma_system_id, true, 'saa' => true));
	else
		$page_options[] = $this->Html->link(__('Add %s ', __('FISMA System %s', $file_type)), array('action' => 'add', $fisma_system_id, 'saa' => true));
}
